...
function toBackendItem(item) {
  ...
  if (!itemId || itemId === 'unknown') return null
...
export async function sendMessage(message, { wardrobeItems = [] } = {}) {
  try {
    const validBackendItems = (wardrobeItems || []).map(toBackendItem).filter(Boolean)
    const res = await fetch(`${API_BASE}/chat`, {
      ...
      body: JSON.stringify({
        ...
        wardrobe_items: validBackendItems.length > 0 ? validBackendItems : null,
      }),
    })
