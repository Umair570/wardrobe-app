...
img_path = "upload images/image 6.jpg"

try:
    # 1. Run SAM2 Segmentation
    ...
    for item in seg_result.get("items", []):
        ...
        print("---- IMAGE 6 OLIVE SHIRT DATA ----")
        for i in range(K):
            vol = (counts[i] / np.sum(counts)) * 100
            sat = saturations[i]
            rgb = centers_uint8[i][::-1] 
            print(f"Cluster {i}: Vol {vol:.1f}% | Saturation {sat:.1f} | RGB {rgb}")
            
        print(f"MAX SATURATION SEEN: {np.max(saturations)}")
